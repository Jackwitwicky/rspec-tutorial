describe 'Doubles' do
  ...
  it "allow studding with a hash argument to #double" do
  ...
  context 'with partial test doubles' do
    ...
    it 'allows stubbing database calls with many mock objects' do
      ...
      expect(Customer.all).to eq([c1,c2])
  ...
  context 'with spying abilities' do
    ...
    it 'can expect any message already sent to a declared spy' do
      ...
      # no need to stub send_invoice with allow
    ...
    context 'using let and a before hook' do
      let(:order) do
        spy('Order', process_line_items: nil, charge_credit_card: true, send_confirmation_email: true)
      end
